...
void init() {				// Convenient way to organize initialization
	...
	static void task_2()			// Button press
	{
		...
		while(1){
			...
			if (gpio_get_level(BUTTON_GPIO) == 0) // If the button has been pressed
			{
	...
	static void task_3() // Alphanumeric display
	{
		while(1)
		{
			// Alpha displays 'UP' when direction is true and 'DOWN' when false
			if (direction)
			{
      ...
      int ret = i2c_master_cmd_begin(I2C_EXAMPLE_MASTER_NUM, cmd4, 1000 / portTICK_RATE_MS);
	...
	void app_main()
	{
		...
		xTaskCreate(task_3, "task_3",1024*4, NULL, configMAX_PRIORITIES, NULL); // display direction
		xTaskCreate(task_1, "task_1",1024*2, NULL, configMAX_PRIORITIES-1, NULL); // LED counting
		xTaskCreate(task_2, "task_2",1024*2, NULL, configMAX_PRIORITIES-2, NULL); // button press
